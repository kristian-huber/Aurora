...
public class ShadowBox {
	...
	private static final float OFFSET = 25;
	...
	protected void update() {
		...
		Vector3f centerNear = Vector3f.add(toNear, Engine.getCamera().getPosition(), null);
		Vector3f centerFar = Vector3f.add(toFar, Engine.getCamera().getPosition(), null);
	...
	private Matrix4f calculateCameraRotationMatrix() {
		...
		rotation.rotate((float) Math.toRadians(-Engine.getCamera().getRotation().y), new Vector3f(0, 1, 0));
		rotation.rotate((float) Math.toRadians(-Engine.getCamera().getRotation().x), new Vector3f(1, 0, 0));
		return rotation;
	}
