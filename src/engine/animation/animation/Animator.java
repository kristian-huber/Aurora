package engine.animation.animation;
...
public class Animator {
	...
	private void increaseAnimationTime() {
		animationTime += Engine.getDelta();
		if (animationTime > currentAnimation.getLength()) {
			if (!looping) {
				this.animationTime = currentAnimation.getLength() * 0.99f;
				updating = false;
			}else {
				this.animationTime = 0;
			}
	 ...
	 * 
	 * @param currentPose
	 *            - a map of the local-space transforms for all the joints for the
	 *            desired pose. The map is indexed by the name of the joint which
	 *            the transform corresponds to.
	 * @param joint
	 *            - the current joint which the pose should be applied to.
	 * @param parentTransform
	 *            - the desired model-space transform of the parent joint for the
	 *            pose.
	 */
	 ...
	 *            - the next keyframe in the animation.
	 * @param progression
	 *            - a number between 0 and 1 indicating how far between the previous
	 *            and next keyframes the current animation time is.
